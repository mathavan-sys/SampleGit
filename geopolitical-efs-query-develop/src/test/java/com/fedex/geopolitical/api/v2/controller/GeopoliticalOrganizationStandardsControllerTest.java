...
@ActiveProfiles("test")
public class GeopoliticalOrganizationStandardsControllerTest {
	private MockMvc mockMvc;

	@Autowired
	private GeoplOrgStdRepository repository;
	...
	public void setup() throws AuthzSecurityException {
		...
		geoplOrgStd.setOrgStdCd("UNES");
		geoplOrgStd.setOrgStdNm("Organisation des Unes");
		geoplOrgStd.setCreatedByUserId("3766268");
		geoplOrgStd.setCreatedTmstp(CommonUtilityTest.getCurrentTimeStamp());
		geoplOrgStd.setLastUpdatedByUserId("3766268");
		geoplOrgStd.setLastUpdatedTmstp(CommonUtilityTest.getCurrentTimeStamp());
		repository.save(geoplOrgStd);
	...
	public void createUpdateRefUomType() throws Exception {

		mockMvc.perform(MockMvcRequestBuilders.get(APIMappingConstants.V2 + APIMappingConstants.GEOPOLITICAL_ORG_STDS)
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)
				.header(GenericConstants.SECURITY_TOKEN, MediaType.TEXT_PLAIN_VALUE)).andExpect(status().isOk())
				.andReturn();

		mockMvc.perform(
				MockMvcRequestBuilders.get(APIMappingConstants.V2 + APIMappingConstants.GEOPOLITICAL_ORG_STD_CD, "UNES")
						.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)
